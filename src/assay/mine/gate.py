...
# pytest's USAGE_ERROR. Measured with pytest 8: selecting a node id that does not resolve
# exits 4 and aborts the whole run before collecting anything - so a red run that reports no
# statuses at all and exits 4 is a run whose target tests do not exist at the parent commit.
# That is evidence of red (the commit *adds* them), not an assay bug, and reading it as a
# harness failure would discard the most common shape of a valid candidate.
...
def _shows_failure(red: TestReport) -> bool:
    ...
    failed, a file that would not collect, or a selection that did not resolve at all.
    """
    if any(status in _FAILING_STATUSES for status in red.statuses.values()):
        return True
    if red.uncollectable:
        return True
    return red.exit_code == _PYTEST_USAGE_ERROR and not red.statuses
